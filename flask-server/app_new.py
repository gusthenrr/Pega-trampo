...
def create_job ():
    ...
    db_write ("""
        INSERT INTO jobs (
            ...
            lat, lng, status, company_info_json,
...
def accept_application (app_id ):
    ...
    # 1. Verificar se a aplicação existe e buscar job_id, candidate_id
        app_rows =db .execute ("SELECT job_id, candidate_id FROM job_applications WHERE id = %s",app_id )
        if not app_rows :
        ...
        job_rows =db .execute ("SELECT title, posted_by_user_id FROM jobs WHERE id = %s",job_id )
        ...
        # 5. Criar sessão de trabalho automaticamente (idempotente)
...
def get_session (application_id ):
    ...
    try :
        rows =db .execute (
        "SELECT * FROM job_sessions WHERE application_id = %s",application_id 
        )
        if not rows :
        ...
        return api_error ("Erro ao buscar sessão",500 )
...
def validate_session (session_id ):
    ...
    try :
        ...
        if completed :
            ...
            return api_ok (message ="ServiÃ§o validado com sucesso!",status ="validated")
        ...
        s ["candidate_id"],
        "A empresa informou que o serviÃ§o nÃ£o foi concluÃ­do.",
        s ["job_id"]
        )
        return api_ok (message ="ServiÃ§o marcado como nÃ£o concluÃ­do.",status ="cancelled")
    except Exception as e :
...
def submit_evaluation ():
    ...
    if not evaluated_id or not job_id or not rating :
        return api_error ("Campos obrigatórios: evaluated_id, job_id, rating.",400 )
    ...
    try :
        ...
        return api_error ("A nota (rating) deve ser um número inteiro.",400 )

    # Verifica se já não avaliou
    existing =db .execute (
    "SELECT id FROM user_evaluations WHERE evaluator_id = %s AND evaluated_id = %s AND job_id = %s",
    user_id ,evaluated_id ,job_id 
    )
    if existing :
        return api_error ("Você já avaliou este candidato para este serviço.",409 )
    ...
    try :
        ...
        db_write ("""
            ...
            VALUES (%s, %s, %s, %s, %s)
        """,user_id ,evaluated_id ,job_id ,rating ,comment )

        # Calculate new average rating
        evals =db .execute ("SELECT rating FROM user_evaluations WHERE evaluated_id = %s",evaluated_id )
        total_evals =len (evals )
        avg_rating =sum (e ["rating"]for e in evals )/total_evals if total_evals >0 else 0 

        db_write ("""
            UPDATE jobs 
            SET professional_rating = %s, professional_reviews = %s, completed_jobs = completed_jobs + 1
            WHERE posted_by_user_id = %s
        """,avg_rating ,total_evals ,evaluated_id )

        db .execute ("COMMIT")
        return api_ok (message ="Avaliação enviada com sucesso!")

    except Exception as e :
        db .execute ("ROLLBACK")
        print (f"Erro ao salvar avaliação: {e }")
        return api_error ("Erro ao processar a avaliação.",500 )
...
def get_user_evaluations (evaluated_id ):
    try :
        ...
        for r in rows :
            ...
            if company_name :
                try :display_name =fernet .decrypt (company_name .encode ()).decode ()
                except :display_name =company_name 
            elif full_name :
                try :display_name =fernet .decrypt (full_name .encode ()).decode ()
                except :display_name =full_name 
        ...
        return api_ok (evaluations =results )
    except Exception as e :
        print (f"Erro ao buscar avaliações: {e }")
        return api_error ("Erro ao buscar avaliações",500 )
